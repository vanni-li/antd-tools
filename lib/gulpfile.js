...
// 清空项目目录的 dist，用webpack编译，配置用项目目录的 webpack.config.js
function dist(done) {
...
// 就地把 ts 转换成 js
// components 里的 js 和 jsx 都在 .gitignore 里，不会提交到 git
gulp.task('tsc', () =>
  compileTs(
    gulp.src(tsFiles, {
      base: cwd,
...
// 通过 gulp 编译 components 目录里内容，包括编译less、图片和svg、ts
...
// 更新 antd-tools 包：npm update antd-tools
gulp.task(
  'update-self',
  gulp.series(done => {
    getNpm(npm => {
